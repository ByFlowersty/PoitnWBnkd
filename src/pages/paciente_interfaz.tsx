...
const Paciente_Interfaz: React.FC = () => {
  ...
  const [capturedImage, setCapturedImage] = useState<string | null>(null); // Stores Data URL temporary
  ...
  const startCamera = async () => {
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        try {
            const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "user" }, audio: false }); // Prioritize user-facing camera
            ...
            setShowCameraModal(false); // Ensure modal closes on error
        }
    } else {
  ...
  const capturePhoto = () => {
    if (videoRef.current && canvasRef.current && cameraStream) {
        ...
        if (context) {
            ...
            setCapturedImage(dataUrl); // Save for preview and upload
            stopCamera(); // Stop camera and close modal after capture
        } else {
            toast.error("No se pudo obtener el contexto del canvas.");
            stopCamera();
        }
    } else {
        toast.error("Referencia de video o canvas no encontrada.");
        stopCamera(); // Still close modal on error
    }
  };
  ...
  const uploadPhoto = async (imageDataUrl: string): Promise<string | null> => {
    ...
    try {
        const fileExt = blob.type.split('/')[1] || 'png'; // Get extension from MIME type
        const fileName = `${user.id}-${Date.now()}.${fileExt}`;
        const filePath = `${fileName}`; // File path within the bucket

        // --- BUCKET NAME: Replace 'patient-photos' with your actual bucket name ---
        const { data, error: uploadError } = await supabase.storage
            .from('patient-photos') // <-- REPLACE 'patient-photos' with your bucket name
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (capturedImage) {
        ...
        if (!photoUrl) {
            // Optional: Stop submission if photo upload fails critical?
            // toast.error("La subida de la foto falló. Inténtalo de nuevo.");
            // return;
        }
    }

    // 2. Insert patient data (including photo URL if successful)
    try {
      const { data: newPatient, error } = await supabase
        ...
        .insert({
          ...
          // Ensure 'Foto_paciente' matches your actual column name in Supabase
          Foto_paciente: photoUrl // Store the public URL or null
        })
        .select().single();
      ...
      // Optional: If insert fails, maybe delete the uploaded photo? Needs more complex logic.
    }
  };
  ...
  const formatDate = (dateString: string | null | undefined): string => { /* ... function unchanged ... */
  ...
  const formatTime = (timeString: string | null | undefined): string => { /* ... function unchanged ... */
        ...
        <div className="min-h-screen bg-gradient-to-br from-primary/5 via-white to-accent/5 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl p-8 max-w-lg w-full border border-gray-100">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-5">
                    ...
                    <div> <label htmlFor="allergies" className="block text-sm font-medium text-gray-700 mb-1.5">Alergias conocidas</label> <textarea id="allergies" name="allergies" value={formData.allergies} onChange={handleFormChange} rows={3} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-colors" placeholder="Ej: Penicilina, Cacahuetes, Polvo..." /> </div>
                     ...
                     <div>
                        ...
                        <div className="mt-1 flex items-center space-x-4">
                            {capturedImage ? (
                                <img src={capturedImage} alt="Foto Capturada" className="h-20 w-20 rounded-full object-cover border-2 border-primary" />
                            ) : (
                                <span className="inline-block h-20 w-20 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center border">
                            ...
                            >
                                <Camera className="inline-block h-5 w-5 mr-1" aria-hidden="true" />
                                {capturedImage ? 'Tomar Otra' : 'Tomar Foto'}
                            </button>
                        </div>
                        {isUploadingPhoto && (
                            <div className="mt-2 flex items-center text-sm text-gray-500">
                                <UploadCloud className="animate-pulse h-4 w-4 mr-1" /> Subiendo foto...
                            </div>
                        )}
                    </div>
                    ...
                    {/* Submit Button */}
                    <div className="pt-4">
                        <button type="submit" disabled={isUploadingPhoto} className="w-full bg-primary text-white py-3 px-4 rounded-lg hover:bg-primary/90 transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 font-semibold text-lg shadow-md hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed">
                            {isUploadingPhoto ? 'Guardando...' : 'Guardar y Continuar'}
                        </button>
                    </div>
                </form>
                 ...
                 {showCameraModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
                        <div className="bg-white p-4 rounded-lg shadow-xl max-w-lg w-full mx-4">
                            ...
                            <div className="relative aspect-video bg-gray-200 rounded overflow-hidden mb-4">
                                ...
                                ></video>
                                {!cameraStream && <div className="absolute inset-0 flex items-center justify-center text-gray-500">Iniciando cámara...</div>}
                            </div>
                            <div className="flex justify-center space-x-4">
                                <button
                                    ...
                                    className="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50"
                                >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      {/* Header */}
      <header className="bg-white shadow-sm sticky top-0 z-30 border-b border-gray-200"> {/* ... Header content ... */}
      ...
      <main className="flex-1 pt-6 pb-24 lg:pb-8">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            {/* Sidebar (Desktop) */}
            <aside className="lg:col-span-3 xl:col-span-2 hidden lg:block"> {/* ... Sidebar content ... */}
              ...
              {currentView === 'home' && (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    ...
                    <div className="bg-gradient-to-br from-primary to-blue-600 rounded-xl shadow-lg p-5 text-white"> {/* Ensure 'primary' is blue in tailwind.config.js, added fallback blue-600 */}
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 cursor-pointer hover:shadow-md transition-shadow group" onClick={() => handleViewChange('appointments')} role="button" tabIndex={0} aria-label="Ver próxima cita">
                        <div className="flex justify-between items-start mb-3"> <div> <p className="text-sm text-gray-500 font-medium">Próxima Cita</p>
                          {/* --- Use patientData.proxima_consulta --- */}
                          <h2 className="text-xl font-bold text-gray-800">{formatDate(patientData?.proxima_consulta)}</h2>
                          {/* Simplified secondary text */}
                          <p className="text-xs text-gray-500 mt-1 truncate">Ver detalles en Calendario</p>
                        </div> <div className="flex-shrink-0 h-11 w-11 bg-gradient-to-br from-accent/80 to-accent rounded-full flex items-center justify-center shadow transition-transform duration-300 group-hover:scale-110"><CalendarIcon className="h-5 w-5 text-white" /></div> </div> <span className="text-xs text-primary font-medium opacity-0 group-hover:opacity-100 transition-opacity">Ver detalles</span>
                    </div>
                    {/* Weather Card */}
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5"> {/* Weather card unchanged */}
                  ...
                  {/* Appointments Table */}
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden"> {/* Appointments table unchanged */}
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                     ...
                     <div className="px-5 py-5 sm:px-6 grid grid-cols-1 sm:grid-cols-3 gap-6">
                        ...
                        <div className="sm:col-span-1 flex flex-col items-center sm:items-start">
                            ...
                            {patientData?.Foto_paciente ? (
                                <img
                                    src={patientData.Foto_paciente}
                                    alt="Foto de perfil"
                                    className="h-32 w-32 rounded-full object-cover border-2 border-gray-200 shadow-sm"
                                    onError={(e) => { e.currentTarget.src = '/placeholder-user.png'; }} // Fallback image
                                />
                            ) : (
                                <div className="h-32 w-32 rounded-full bg-gray-100 flex items-center justify-center border">
                                    <User className="h-16 w-16 text-gray-400" />
                                </div>
                            )}
                            {/* Optional: Add button to change photo later */}
                        </div>
                        ...
                        <dl className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                            {[
                                ...
                                <div key={item.label} className={`${item.label === 'Alergias' ? 'sm:col-span-2' : 'sm:col-span-1'}`}>
