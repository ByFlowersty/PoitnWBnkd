...
// VITE_OPENMETEO_API_ENDPOINT=https://api.open-meteo.com/v1/forecast
// VITE_EREBUS_API_ENDPOINT=https://erebus-production.up.railway.app // <-- ¡Usa HTTPS!

// Uso (ejemplo):
const openMeteoApiEndpoint = import.meta.env.VITE_OPENMETEO_API_ENDPOINT || 'https://api.open-meteo.com/v1/forecast';
// Nota: supabaseClient ya debería estar configurado con las variables de entorno
...
const Paciente_Interfaz: React.FC = () => {
  ...
  const [loyaltyCode, setLoyaltyCode] = useState<string>(''); // Código generado localmente
  ...
  useEffect(() => {
    const checkAuthAndPatientData = async () => {
      ...
      try {
        ...
        // Verificar si existe el perfil del paciente
        const { data: patient, error: patientError } = await supabase
          .from('patients')
          .select('*')
          .eq('user_id', authUser.id)
          .maybeSingle(); // Use maybeSingle para manejar 0 o 1 resultado sin error

        if (patientError) {
           console.error("Error fetching patient data:", patientError);
           throw new Error("Error al cargar datos del paciente."); // Lanzar error para el catch
        }

        if (!patient) {
          // Si no hay paciente, mostrar formulario para crearlo
          setShowPatientForm(true);
          setLoading(false);
        } else {
          // Si hay paciente, establecer datos y cargar citas
          setPatientData(patient);
          setLoyaltyCode(patient.surecode || ''); // Cargar código existente
          setShowPatientForm(false);
          setLoading(false);
          fetchAppointments(authUser.id); // Cargar citas después de tener datos del paciente
        }
        ...
        setLoading(false);
        // Considerar redirigir a login o mostrar un mensaje de error persistente
      }
    };

    checkAuthAndPatientData();
  }, [showToast]); // Incluir showToast si se usa dentro
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Actualizar estado local y cerrar formulario
      setPatientData(newPatient);
      setShowPatientForm(false);
      showToast('Perfil creado exitosamente.', 'success');
      fetchAppointments(user.id); // Cargar citas para el nuevo perfil
  ...
  const generateLoyaltyCode = async () => {
    ...
    try {
      const characters = 'ABCDEFGHIJKLMNPQRSTUVWXYZ123456789'; // Caracteres más legibles
      const codeLength = 10;
      let result = '';
      ...
      setLoyaltyCode(result); // Actualizar estado local
      setPatientData((prev: any) => ({ ...prev, surecode: result })); // Actualizar datos del paciente
  ...
  const fetchAppointments = async (patientId: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        // ***** CORRECCIÓN AQUÍ *****
        .order('appointment_date', { ascending: true }); // Quitado el ':1'

      if (error) throw error;

      const processedAppointments = data?.map(appt => ({
        ...appt,
        doctor_name: appt.doctors?.name || 'Dr. Asignado'
      })) || [];
      setAppointments(processedAppointments);

    } catch (err: any) {
      console.error('Error fetching appointments:', err);
      // No mostrar toast aquí para no ser muy intrusivo, el estado vacío en la tabla ya indica algo
      // showToast(`Error al cargar citas: ${err.message}`, 'error');
    } finally {
  ...
  useEffect(() => {
    const fetchWeather = () => {
      ...
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        try {
          // ***** CORRECCIÓN AQUÍ *****
          // Usar endpoint de variable de entorno o el valor por defecto
          ...
          if (data?.current) {
            ...
            setWeatherData({
              ...
              icon: details.icon,
              location: 'Tu ubicación', // Open-Meteo no da ciudad fácilmente
              day: new Date().toLocaleDateString('es-ES', { weekday: 'long' }),
            });
          } else {
      ...
      }, (geoError) => { // Manejo de error de geolocalización
  ...
  const formatDate = (dateString: string | null | undefined): string => {
    ...
    try {
      ...
      // Asumiendo que la fecha de Supabase es UTC o ya tiene offset correcto
  ...
  if (error) {
    return (
       <div className="min-h-screen flex flex-col items-center justify-center bg-red-50 p-4 text-center">
           ...
           <button
             onClick={() => window.location.reload()} // Opción simple: recargar
  ...
  if (showPatientForm) {
    // --- Formulario de Creación de Perfil (Rediseñado) ---
    return (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-colors bg-white appearance-none" // appearance-none para estilo custom
                >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-100">
      <ToastProvider /> {/* Asegúrate que esté fuera de condicionales */}
      ...
      <header className="bg-white shadow-sm sticky top-0 z-30 border-b border-gray-200">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
          ...
          {/* Botón Menú Móvil */}
          <button
            className="p-2 rounded-md text-gray-600 hover:text-primary hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary lg:hidden" // lg:hidden para mostrar solo en móvil/tablet
      ...
      {/* --- Contenido Principal --- */}
      <main className="flex-1 pt-6 pb-24 lg:pb-8"> {/* Padding inferior mayor en móvil por bottom nav */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            <aside className="lg:col-span-3 xl:col-span-2 hidden lg:block">
              <div className="sticky top-20 bg-white rounded-xl shadow-sm border border-gray-200 p-4 space-y-1.5">
                ...
                {[
                  ...
                  { view: 'EREBUS', label: 'EREBUS', icon: FileText }, // Reusando icono por ahora
                  { view: 'pharmacies', label: 'Farmacias', icon: Package2 },
                  { view: 'profile', label: 'Perfil', icon: User }, // Cambiado a User para Perfil
                ].map(item => (
                  <button
                    key={item.view}
            ...
            <div className="lg:col-span-9 xl:col-span-10 space-y-6">
              ...
              {currentView !== 'home' && currentView !== 'profile' && (
                <ContentPanel
                  view={currentView as any} // Forzar tipo si es necesario, o ajustar tipos en ContentPanel
                  patientId={user?.id} // Pasar ID del paciente si los componentes lo necesitan
                  // patientData={patientData} // Pasar datos completos si es necesario
                  // appointments={appointments} // Pasar citas si es necesario
                  // fetchAppointments={() => fetchAppointments(user?.id)} // Pasar función para recargar
                  onClose={() => handleViewChange('home')} // Función para volver (usada en Farmacias)
                />
              )}
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 md:p-6">
                     <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
                       <div>
                         ...
                         <p className="text-2xl font-bold text-primary font-mono tracking-widest bg-gray-100 px-4 py-2 rounded-md inline-block">
                      ...
                      {(patientData?.surecode || loyaltyCode) && showBarcode && (
                        <div className="mt-6 p-4 bg-white rounded-lg border border-gray-200 overflow-x-auto max-w-md mx-auto flex justify-center">
                          <Barcode
                            ...
                            displayValue={false} // Se muestra arriba ya
                     ...
                     <div className="px-5 py-5 sm:px-6">
                       <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                         ...
                         {[
                           ...
                           // Puedes añadir más campos aquí si existen en patientData
                         ].map(item => (
                           <div key={item.label} className="sm:col-span-1">
                             <dt className="text-sm font-medium text-gray-500">{item.label}</dt>
                             <dd className="mt-1 text-sm text-gray-900">{item.value || 'No disponible'}</dd>
                           </div>
                         ))}
                       </dl>
                     </div>
